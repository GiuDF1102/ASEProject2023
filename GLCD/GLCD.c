...
void LCD_DrawEmptyRectangle(uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t borderColor)
{
	uint16_t i, j;
	...
	for (i = 0; i < width; i++) {
		for (j = 0; j < height; j++) {
			if(j == 0 || (j == height - 1)) {
				LCD_SetPoint(X_start + i, Y_start + j, borderColor);				
